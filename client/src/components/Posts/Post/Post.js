...
// const postCreat=user?.result?.googleId === post?.creator || user?.result?._id === post?.creator;
  ...
  return (
    <Card className={classes.card} raised elevation={6}>
      ...
      >
        ...
        <Typography className={classes.title} gutterBottom variant="h6" component="h2">Title: {post.title}</Typography>
        <CardContent>
          <Typography variant="body2" color="textSecondary" component="p"> Message:m{post.message.split(' ').splice(0, 20).join(' ')}...</Typography>
        </CardContent>
      </ButtonBase>
      <CardActions className={classes.cardActions}>
        ...
        {(user?.result?.googleId === post?.creator || user?.result?._id === post?.creator) && (
          <Button size="small" color="secondary" onClick={() => dispatch(deletePost(post._id))}>
            <ClearIcon fontSize="small" /> &nbsp; Delete
          </Button>
        )}
      </CardActions>
    </Card>
